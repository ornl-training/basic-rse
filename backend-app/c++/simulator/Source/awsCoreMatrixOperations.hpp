...
//! \ingroup AwesomeSLA
template<typename typeFP, typename typeIdx = int>
void spFactorizeILU(const std::valarray<typeIdx> &pntr, const std::valarray<typeIdx> &indx, const std::valarray<typeFP> &vals,
                    std::valarray<typeIdx> &diag, std::valarray<typeFP> &ilu){
...
//! - \b ilu will hold the values of the upper and lower factors
    ...
    for(typeIdx i=0; i<num_rows; i++){
        typeFP pivot = ilu[diag[i]]; //std::cout << pivot << std::endl;
